...
def debugTestDataset(lmDataset):
    ...
    showLinemodFrame(frame,showPlot=False)
...
#         cv2.imshow("train{}".format(cl),img)
    ...
    plotAngleErrors(sim,dst,fileName=[pdfFN,pngFN],tikzFileName=tikzFN,pklFileName=pklFN,showPlot=False)
...
#     cv2.imshow("maxSim",(sim == maxSim).astype(numpy.float))
    ...
    visClosestTmpls(testdata_set,tmpl_set,dst,fileName=outFNBase+'_samples.png',showPlot=False)

    #visWrongestClosestTmpls(testdata_set,tmpl_set,dst,sim,fileName=outFNBase+'_wrongestsamples.png',showPlot=showPlots)
    visWrongestClosestTmpls(testdata_set,tmpl_set,dst,sim,fileName=outFNBase+'_wrongestsamples.png',showPlot=False)
